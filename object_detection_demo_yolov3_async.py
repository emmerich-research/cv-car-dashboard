    ...
    while (cap.isOpened() \
        ...
        if next_frame_id_to_show in completed_request_results:
            ...
            for obj in objects:
                ...
                # If object can be categorized to one of the coco_names
                beep_thread = threading.Thread(target=beep)
                if obj['class_id'] in coco_names.values():
                    # Beep the alarm
                    beep_thread.start()
                ...
                cv2.rectangle(frame, (obj['xmin'], obj['ymin']), (obj['xmax'], obj['ymax']), color, thickness)
                cv2.putText(frame,
